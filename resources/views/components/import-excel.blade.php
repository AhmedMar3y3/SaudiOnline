        ...
        <div class="modal-content">
            ...
            <div class="modal-body text-danger">
                ...
                <form action="{{ route('dashboard.excel.import') }}"
                    ...
                    @if($model === \Modules\SpareParts\Entities\SparePart::class)
                        @if(auth()->user()->isProvider())
                            <input type="hidden" name="provider_id" value="{{ auth()->id() }}">
                        @else
                            @if (\Module::collections()->has('Accounts'))
                                <select2 name="provider_id"
                                         label="@lang('accounts::providers.attributes.name')"
                                         remote-url="{{ route('providers.select') }}"
